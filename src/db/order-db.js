...
class OrderDB {
  ...
  _migrate() {
    ...
    // ★ 재고 마스터 (엑셀 업로드)
    this.db.run(`CREATE TABLE IF NOT EXISTS inventory (id INTEGER PRIMARY KEY AUTOINCREMENT, product_code TEXT NOT NULL, barcode TEXT DEFAULT '', product_name TEXT NOT NULL, option_name TEXT DEFAULT '', category TEXT DEFAULT '', supplier TEXT DEFAULT '', supplier_option TEXT DEFAULT '', cost_price REAL DEFAULT 0, sell_price REAL DEFAULT 0, stock_qty INTEGER DEFAULT 0, defect_qty INTEGER DEFAULT 0, base_date TEXT DEFAULT '', uploaded_at TEXT DEFAULT (datetime('now')), UNIQUE(product_name, option_name))`);
  ...
  _refreshDailySummary(channel) {
    ...
    this.db.run(`INSERT INTO daily_summary (date, channel, order_count, total_amount, avg_order) SELECT order_date, channel, COUNT(DISTINCT order_id), SUM(amount), CASE WHEN COUNT(DISTINCT order_id)>0 THEN ROUND(SUM(amount)/COUNT(DISTINCT order_id)) ELSE 0 END FROM orders WHERE channel = ? AND order_date != '' GROUP BY order_date, channel`, [channel]);
  }

  // 매출 조회 (기존 유지)
  getSalesSummary(s, e) { return this.db.exec(`SELECT channel,COUNT(DISTINCT order_id),COALESCE(SUM(amount),0),CASE WHEN COUNT(DISTINCT order_id)>0 THEN ROUND(SUM(amount)/COUNT(DISTINCT order_id)) ELSE 0 END FROM orders WHERE order_date BETWEEN '${s}' AND '${e}' AND status NOT LIKE 'C%' AND status NOT LIKE 'R%' AND status!='CANCELED' AND status!='RETURNED' GROUP BY channel`)[0]?.values?.map(r=>({channel:r[0],order_count:r[1],total_amount:r[2],avg_order:r[3]}))||[]; }
  getDailySales(s, e, ch) { let q=`SELECT order_date,channel,COUNT(DISTINCT order_id),COALESCE(SUM(amount),0) FROM orders WHERE order_date BETWEEN '${s}' AND '${e}' AND status NOT LIKE 'C%' AND status NOT LIKE 'R%' AND status!='CANCELED' AND status!='RETURNED'`; if(ch&&ch!=='all')q+=` AND channel='${ch}'`; q+=' GROUP BY order_date,channel ORDER BY order_date'; return this.db.exec(q)[0]?.values?.map(r=>({date:r[0],channel:r[1],order_count:r[2],total_amount:r[3]}))||[]; }
  ...
  getOrderCount() { return this.db.exec('SELECT COUNT(DISTINCT order_id) FROM orders')[0]?.values?.[0]?.[0]||0; }
  getOrderCountByChannel() { return this.db.exec('SELECT channel,COUNT(DISTINCT order_id) FROM orders GROUP BY channel')[0]?.values?.map(r=>({channel:r[0],count:r[1]}))||[]; }
