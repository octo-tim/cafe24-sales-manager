...
//  설정
// ─────────────────────────────────────────────
const CONCURRENCY       = 5;    // 동시 API 요청 수 (카페24 권장: 초당 5)
const MIN_DELAY_MS      = 120;  // 요청 간 최소 간격 (ms)
const RATE_LIMIT_DELAY  = 1200; // 429 응답 시 대기 (ms)
const PAGE_SIZE         = 100;  // 페이지당 주문 수 (카페24 최대값)
const CACHE_TTL_MS      = 60_000; // 캐시 유효시간 (1분)
...
class Cafe24Client {
  constructor(config = {}) {
    ...
    // Rate limit 상태 (헤더 기반으로 업데이트)
    this._bucket = { remaining: 10, limit: 10 };

    // 간단한 메모리 캐시 { key → { data, expires } }
    this._cache = new Map();
  }
   ...
   * 기간 내 전체 주문 병렬 조회
   *  1) 건수 조회 → 총 페이지 계산
   *  2) 모든 페이지 병렬 fetch (CONCURRENCY 제한)
   */
  async getAllOrders(startDate, endDate, extraParams = {}) {
    ...
    // ── 1. 전체 건수 조회
    const countRes = await this._get('/api/v2/admin/orders/count', {
      start_date: startDate,
      end_date:   endDate,
    });
    ...
    // ── 3. CONCURRENCY 제한 병렬 실행
    const allOrders = await this._parallelPages(offsets, (offset) =>
      this._get('/api/v2/admin/orders', { ...baseParams, limit: PAGE_SIZE, offset })
        ...
        .catch((e) => {
          console.error(`[Cafe24] offset=${offset} 오류: ${e.message}`);
          return [];
        })
    );

    const flat = allOrders.flat();
  ...
  async getDashboard() {
    return this._get('/api/v2/admin/dashboard');
  }
  ...
  async getSalesAnalytics(startDate, endDate) {
    ...
    for (const o of orders) {
      ...
      const amt = parseFloat(o.actual_payment_amount || o.total_amount_paid || 0);
      totalRevenue += amt;
      ...
      for (const item of (o.items || [])) {
        ...
        if (!productSales[k])
          productSales[k] = { product_no: item.product_no, product_name: item.product_name, quantity: 0, revenue: 0 };
  ...
  /**
   * Rate limit 헤더 기반 동적 대기 후 요청
   * 429 응답 시 자동 재시도 (최대 3회)
   */
  async _apiRequest(opts, retries = 3) {
    ...
    try {
      ...
      if (e.statusCode === 429 && retries > 0) {
        console.warn(`[Cafe24] 429 Rate limit → ${RATE_LIMIT_DELAY}ms 대기 후 재시도 (남은 횟수: ${retries})`);
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = https.request(reqOpts, (res) => {
        ...
        res.on('end', () => {
          ...
          if (lh) {
            const [used, total] = lh.split('/').map(Number);
            this._bucket = { remaining: total - used, limit: total };
          }

          try {
            const parsed = JSON.parse(d);
            if (res.statusCode === 429) {
              const e = new Error('Too Many Requests'); e.statusCode = 429; return reject(e);
            }
            if (res.statusCode >= 400) {
              const msg = parsed?.error?.message || `API ${res.statusCode}`;
              const e   = new Error(msg); e.statusCode = res.statusCode; return reject(e);
            }
            resolve(parsed);
          } catch (e) {
            reject(new Error('Response parse fail: ' + d.substring(0, 200)));
          }
        });
      });
      ...
      const req = https.request({
        ...
        res.on('end', () => {
          try {
            ...
            res.statusCode >= 400 ? reject(new Error(`Auth ${res.statusCode}: ${JSON.stringify(p)}`)) : resolve(p);
  ...
  _saveTokens() {
    try {
      fs.writeFileSync(path.resolve(this.config.tokenStorePath), JSON.stringify(this.tokens, null, 2), 'utf8');
    } catch (e) { /* 무시 */ }
  }
